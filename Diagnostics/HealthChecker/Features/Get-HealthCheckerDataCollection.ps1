...
function Get-HealthCheckerDataCollection {
    ...
    process {
        ...
        foreach ($serverName in $getExchangeServerList.Keys) {
            ...
            if ($orgRunType -eq "Legacy" -or $null -ne $orgCmdletJobResults) {
                if ($null -eq $orgCmdletJobResults) {
                    throw "Organization Cmdlet Job Results NULL from Legacy Option"
                }
            ...
            if ($exchCmdletRunType -eq "Legacy" -or $null -ne $exchCmdletJobResults) {
